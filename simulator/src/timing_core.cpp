...
void TimingCore::initStats(AggregateStat *parentStat)
{
    ...
    auto y = [this]()
    { return cRec.getContentionCycles(); };
...
void TimingCore::loadAndRecord(Address addr, uint32_t size, Address pc)
{
...
void TimingCore::storeAndRecord(Address addr, uint32_t size, Address pc)
{
    uint64_t startCycle = curCycle;
...
void TimingCore::bblAndRecord(Address bblAddr, BblInfo *bblInfo)
{
    ...
    for (Address fetchAddr = bblAddr; fetchAddr < endBblAddr; fetchAddr += (1 << lineBits))
    {
        ...
        curCycle = l1i->load(info, curCycle); // Fetch instruction doesn't have a PC
        cRec.record(startCycle);
    }
}
...
void TimingCore::LoadAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT pc)
{
    static_cast<TimingCore *>(cores[tid])->loadAndRecord(addr, size, pc);
}

void TimingCore::StoreAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT pc)
{
    static_cast<TimingCore *>(cores[tid])->storeAndRecord(addr, size, pc);
}
...
void TimingCore::PredLoadAndRecordFunc(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, ADDRINT pc)
{
    if (pred)
        static_cast<TimingCore *>(cores[tid])->loadAndRecord(addr, size, pc);
}

void TimingCore::PredStoreAndRecordFunc(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, ADDRINT pc)
{
    if (pred)
        static_cast<TimingCore *>(cores[tid])->storeAndRecord(addr, size, pc);
}
