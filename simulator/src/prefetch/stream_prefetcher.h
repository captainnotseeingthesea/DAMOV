...
public:
	...
	StreamPrefetcherEvent(int32_t domain, uint64_t startMinCycle, EventRecorder *_evRec) : TimingEvent(0, 0, domain),
																						   evRec(_evRec)
	{
...
public:
	explicit StreamPrefetcher(const g_string &_name, const int nlines, const uint32_t _nEntries) : timestamp(0), pfEntries(_nEntries), name(_name)
	{
		createArray();
	}
