...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, Address pc, int32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId)
{
    ...
    case E:
    {
        AccessInfo accessInfo = {0, pc, 0, AccessInfo::NONE, AccessInfo::OTHER};
        MemReq req = {wbLineAddr, accessInfo, PUTS, selfId, state, cycle, &ccLock, *state, srcId, 0 /*no flags*/};
    ...
    {
        AccessInfo accessInfo = {0, pc, 0, AccessInfo::NONE, AccessInfo::OTHER};
        MemReq req = {wbLineAddr, accessInfo, PUTX, selfId, state, cycle, &ccLock, *state, srcId, 0 /*no flags*/};
        respCycle = parents[getParentId(wbLineAddr)]->access(req);
    }
    break;
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, AccessInfo accessInfo, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags)
{
    uint64_t respCycle = cycle;
    MESIState *state = &array[lineId];
    EntryInfo *entry = &entries[lineId];

    if (bypass)
    {
        uint32_t parentId = getParentId(lineAddr);
        MemReq req = {lineAddr, accessInfo, type, selfId, state, cycle, &ccLock, *state, srcId, flags};
        respCycle = parents[parentId]->access(req); // We send the request to the next level
        entry->insert(respCycle, flags & MemReq::PREFETCH);
        return respCycle;
    }
    ...
    case GETS:
        ...
        {
            uint32_t parentId = getParentId(lineAddr);
            MemReq req = {lineAddr, accessInfo, GETS, selfId, state, cycle, &ccLock, *state, srcId, flags};
            ...
            entry->insert(respCycle, flags & MemReq::PREFETCH);
            profGETSMiss.inc();
            if(accessInfo.isGraphData()) profGraphGETSMiss.inc();
            // assert_msg(*state == S || *state == E, "state = %d, %s", *state, name.c_str());
        }
        else
        {
            respCycle = MAX(respCycle, entry->getReadyTick()); 
            entry->clearPrefetched();
            profGETSHit.inc();
            if(accessInfo.isGraphData())
            {
                profGraphGETSHit.inc();
            }
        }
    ...
    case GETX:
        if (*state == I || *state == S)
        {
            ...
            {
                profGETXMissIM.inc();
                if(accessInfo.isGraphData()) profGraphGETXMissIM.inc();
            }
            else
            {
                profGETXMissSM.inc();
                if(accessInfo.isGraphData())  profGraphGETXMissSM.inc();
            }
            uint32_t parentId = getParentId(lineAddr);
            MemReq req = {lineAddr, accessInfo, GETX, selfId, state, cycle, &ccLock, *state, srcId, flags};
            uint32_t nextLevelLat = parents[parentId]->access(req) - cycle;
            uint32_t netLat = (network) ? network->getRTT(cycle, nextLevelLat, name.c_str(), parents[parentId]->getName()) : 0;
            profGETNextLevelLat.inc(nextLevelLat);
            profGETNetLat.inc(netLat);
        ...
        else
        {
            ...
            profGETXHit.inc();
            if(accessInfo.isGraphData()) profGraphGETXHit.inc();
            respCycle = MAX(respCycle, entry->getReadyTick()); 
            entry->clearPrefetched();
        }
...
uint64_t MESIBottomCC::processNonInclusiveWriteback(Address lineAddr, Address pc, AccessType type, uint64_t cycle, MESIState *state, uint32_t srcId, uint32_t flags)
{
    ...
    AccessInfo accessInfo = {0, pc, 0, AccessInfo::NONE, AccessInfo::OTHER};
    MemReq req = {lineAddr, accessInfo, type, selfId, state, cycle, &ccLock, *state, srcId, flags | MemReq::NONINCLWB};
...
{
    if (type == FWD)
    {                              // if it's a FWD, we should be inclusive for now, so we must have the line, just invLat works
