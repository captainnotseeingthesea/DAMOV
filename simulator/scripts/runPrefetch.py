...
FRAMEWORK_TASKS = [
        ...
        # 不带 filter 的 graph prefetch 配置
        ("kickstarter", f"graph_prefetch_{root}_{dest}_{EXPERIMENT_CONFIG['prefetch']['PDELatency']}")
        for root, dest in EXPERIMENT_CONFIG["prefetch"]["graphPrefetchEntries"]
    ] + [
        # 带 filter 的 graph prefetch 配置
        ("kickstarter", f"graph_prefetch_with_filter_{root}_{dest}_{EXPERIMENT_CONFIG['prefetch']['PDELatency']}")
        for root, dest in EXPERIMENT_CONFIG["prefetch"]["graphPrefetchEntries"]
    ]
